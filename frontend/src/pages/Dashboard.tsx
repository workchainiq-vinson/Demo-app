...
export default function Dashboard() {
  ...
  if (error || !summary) {
    return <div className="rounded-lg bg-red-50 px-4 py-3 text-sm text-red-700">{error ?? "No data available."}</div>;
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm lg:col-span-2">
          <p className="mb-3 text-sm font-semibold text-slate-700">Payroll Trend (Gross vs Net)</p>
          {chartData.length === 0 ? (
            <p className="py-10 text-center text-sm text-slate-400">No payroll runs yet.</p>
          ) : (
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={chartData}>
                ...
                <Legend />
                <Bar dataKey="Gross" fill="#2f5233" radius={[4, 4, 0, 0]} />
                <Bar dataKey="Net" fill="#7fb069" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </div>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
          <p className="mb-3 text-sm font-semibold text-slate-700">Upcoming Holidays</p>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
          <div className="rounded-lg border border-slate-100 p-3">
            ...
            <div className="flex gap-2">
              <a
                ...
                className={`flex flex-1 items-center justify-center gap-1 rounded-lg border border-slate-300 px-2 py-1.5 text-xs font-medium ${
          ...
          <div className="rounded-lg border border-slate-100 p-3">
            <p className="mb-2 text-xs font-medium text-slate-600">Attendance Report</p>
          ...
          <div className="rounded-lg border border-slate-100 p-3">
            <p className="mb-2 text-xs font-medium text-slate-600">Pakyaw Production Report</p>
            <div className="mb-2 flex gap-1">
              <input
                type="date"
                value={pakyawFrom}
          ...
          <div className="rounded-lg border border-slate-100 p-3">
            <p className="mb-2 text-xs font-medium text-slate-600">Employee Directory</p>
            <p className="mb-2 text-xs text-slate-400">{employees.length} employees on record</p>
            <a
