...
export default function PayrollGeneration() {
  ...
  return (
    <div>
      ...
      <form onSubmit={handleGenerate} className="mb-4 flex flex-wrap items-end gap-3 rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
        <div>
            ...
            {loading ? (
              ...
              selectedRun.payslips.map((p) => (
                <tr key={p.id} className="hover:bg-slate-50">
                  <td className="px-3 py-2 font-medium text-slate-900">{p.employee_name}</td>
                  <td className="px-3 py-2">{formatPeso(p.gross_pay)}</td>
